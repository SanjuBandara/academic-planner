...
public class BurnoutDetectorTool {
        ...
        public BurnoutReport analyse(User user) {
                ...
                List<StudyPlanItem> items = studyPlanItemRepository.findItemsInDateRange(user.getId(), twoWeeksAgo,
                                today);
                ...
                long completed = items.stream().filter(i -> i.getStatus() == StudyPlanItem.ItemStatus.COMPLETED)
                                .count();
                ...
                double completedHours = items.stream()
                                ...
                                .mapToDouble(i -> i.getActualHours() > 0
                                                ? i.getActualHours()
                                                : i.getPlannedHours())
                                .sum();
                ...
                String summary = String.format(
                                ...
                                total, totalHours, completed, completedHours, skipped, planned, completionRate,
                                skippedRate);
                ...
                if (burnoutDetected) {
                        recommendation = "⚠️ High skip rate detected (" + String.format("%.0f%%", skippedRate) + "). "
                                        ...
                                        + (mostSkippedDay != null
                                                        ? " You tend to skip most on **" + mostSkippedDay
                                                                        + "** — consider keeping that day lighter."
                                                        : "");
                } else if (completionRate >= 80) {
                        recommendation = "🌟 Excellent consistency! You're completing "
                                        + String.format("%.0f%%", completionRate)
                                        + " of your planned sessions. Keep it up!";
                } else if (completionRate >= 50) {
                        recommendation = "👍 Good progress (" + String.format("%.0f%%", completionRate)
                                        + " completion rate). "
